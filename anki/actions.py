    ...
    params={
        "note": {
            ...
            "fields": {
                ...
                "Tense": verb.tense,
                **{p.capitalize(): verb.conjugation.get(p.lower(), "").split(" / ")[0].strip() for p in PRONOUNS},
                "FormVariant": " | ".join(f"{p}: {v}" for p, v in verb.conjugation.items() if " / " in v),
                "Arab": verb.arab
            },
            "options": {
            ...
            "tags": [
                f"type::verb verb::tense::{verb.tense}"
                    ]
            }
        }
